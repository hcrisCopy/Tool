...
case "${STAGE_START}" in
  fresh|behavior|statistics)
    ;;
  *)
    echo "STAGE_START must be fresh, behavior, or statistics" >&2
    exit 2
    ;;
esac
...
if [[ "${STAGE_START}" != statistics ]]; then
